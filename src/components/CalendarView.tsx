...
export function CalendarView({ events, onEventClick }: CalendarViewProps) {
    ...
    const getStatusColor = (status: string) => {
        ...
        if (['delivered', 'Entregue', 'Em Entrega'].includes(status)) return 'bg-muted text-muted-foreground border-border';
    ...
    return (
        <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden h-full flex flex-col">
            ...
            {/* Days Header */}
            <div className="grid grid-cols-7 bg-muted border-b border-border flex-none">
            ...
            {/* Calendar Grid */}
            <div className="grid grid-cols-7 auto-rows-fr bg-border gap-px flex-1 overflow-y-auto">
                {days.map((day) => {
                    ...
                    return (
                        <div
                            key={day.toString()}
                            className={`min-h-[100px] bg-card p-2 transition-colors hover:bg-accent flex flex-col ${!isSameMonth(day, monthStart) ? 'bg-muted/50 text-muted-foreground' : ''
                                }`}
                        >
