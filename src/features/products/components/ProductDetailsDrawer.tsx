...
const ProductDetailsDrawer: React.FC<ProductDetailsDrawerProps> = ({
  ...
  return (
    <AppDialog
      ...
      size="lg"
      title={product.name}
      description={product.product_type === 'COMPONENTE_BOLO' ? 'Componente usado na montagem de bolos personalizados.' : 'Produto exibido no catalogo do menu.'}
      className="border-b border-border/70 px-6 py-4 text-left"
      contentClassName="!overflow-hidden w-full max-w-full rounded-none border-0 bg-card p-0 sm:rounded-3xl md:border shadow-2xl grid grid-rows-[auto,1fr]"
    >
            ...
            <div className="my-1 h-px bg-border/70" />
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  {product.is_public ? 'Ocultar' : 'Tornar visivel'}
                </Button>
