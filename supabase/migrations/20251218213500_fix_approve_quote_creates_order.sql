-- Migration: Update approve_quote_via_token RPC to also create order
-- This ensures that when a client approves a quote via public link,
-- the order is created within the database function (bypassing RLS)
-- First, drop the existing function if it exists
DROP FUNCTION IF EXISTS approve_quote_via_token(text);
-- Create the updated function that also creates the order
CREATE OR REPLACE FUNCTION approve_quote_via_token(input_token text) RETURNS quotes LANGUAGE plpgsql SECURITY DEFINER -- This allows the function to bypass RLS
    AS $$
...
-- Log the order creation (optional, for debugging)
RAISE NOTICE 'Order created with ID: %',
new_order_id;
END IF;
RETURN target_quote;
END;
$$;
-- Grant execute permission to anonymous users (for public quote approval)
